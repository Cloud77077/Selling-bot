...
def save_product(name: str = Form(), price: float = Form(), description: str = Form(''), image: UploadFile | None = File(None), db: Session = Depends(get_db), admin=Depends(current_admin)):
    ...
    if image and image.filename:
        os.makedirs(get_settings().upload_dir, exist_ok=True); path = f'/media/uploads/{image.filename}'
...
@app.post('/settings')
def settings_save(upi_id: str = Form(''), binance_pay_qr_path: str = Form(''), low_stock_threshold: str = Form('5'), order_expiry_minutes: str = Form('30'), db: Session = Depends(get_db), admin=Depends(current_admin)):
    for k,v in dict(upi_id=upi_id, binance_pay_qr_path=binance_pay_qr_path, low_stock_threshold=low_stock_threshold, order_expiry_minutes=order_expiry_minutes).items(): db.merge(Setting(key=k,value=v))
